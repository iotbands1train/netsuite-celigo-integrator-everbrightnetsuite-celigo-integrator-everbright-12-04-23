...
define(['N/record', 'N/search', 'N/task', './BB.SS.MD.CreateAddressRecord', './BB_SS_MD_SolarConfig', './BB.SS.ScheduledScript.BatchProcessing',
    ...
    function (record, search, task, addressRecord, solarConfig, batchProcessor, docLib, documentStatusLib, getLocation) {
        ...
        function ConfigOptions() {
            ...
            obj.usesubs = configcheck.custrecord_bb_ss_has_subsidiaries;
            obj.useAddressRecord = configcheck.custrecord_bb_exclud_addy_record_on_proj;
            log.debug('obj', obj);
            return obj;
        }
        ...
        */
        function transformEntityToProject(entityId, noProjectActions, noSave) {
            if (entityId) {
                var requiredFieldList = getProjectRequiredFieldRecords();
                var entityObj = getEntityRecordData(entityId, requiredFieldList);
                var projectId = createProjectRecord(entityObj, noProjectActions, noSave);
                return projectId;
            }
        }

        /*
         Create project by lot number.
         @param {data} - Contains internal Id of parent record.
         @param {lotNumbers} Lot number of project being created.
         @returns Project Internal ID after successful project creation
         */
        function createProjectByLotNumber(data, lotNumbers, noProjectActions, noSave) {
            var project;
            var entityId = data.parent;
            var subdivisionName = data.subdivision;
            log.debug('subdivisionName', subdivisionName);
            if (entityId) {
                log.debug('lotNumbers', lotNumbers);
                var requiredFieldList = getProjectRequiredFieldRecords();
                var entityObj = getEntityRecordData(entityId, requiredFieldList);
                log.debug('lotNumbers', lotNumbers);
                entityObj.custentity_bb_install_address_1_text = subdivisionName;
                entityObj.custentity_bb_install_address_2_text = lotNumbers;

                project = createProjectRecord(entityObj, noProjectActions, noSave);
                //setup company name details here
                project.setValue({
                    fieldId: 'companyname',
                    value: subdivisionName + ' ' + lotNumbers,
                    ignoreFieldChange: true
                });
                log.debug('project object at line 105', project)
                var id = project.save({ ignoreMandatoryFields: true });
                try {
                    paramObj = paramObj ? paramObj : setProjectRelatedFields(project, entityObj);
                    var taskParameters = {};
        ...
        */
        var paramObj;
        function createProjectRecord(entityObj, noProjectActions, noSave) {
            if (entityObj) {
                // check for required fields here
                var missingRequiredFieldsObj = validateRequiredProjectFields(entityObj);
                var missingRequiredFields = missingRequiredFieldsObj.requiredValues;
                var projectRequiredFieldNotSet = missingRequiredFieldsObj.projectRequiredFieldNotSet;
                log.debug('missingRequiredFields', missingRequiredFields);
                log.debug('missingRequiredFieldsObj', missingRequiredFieldsObj);
                if (missingRequiredFields.length > 0) {
                    var errorMessage = printRequiredFieldErrorMessage(missingRequiredFields, projectRequiredFieldNotSet);
                    throw errorMessage;
                } else if (projectRequiredFieldNotSet) {
                    var errorMessage = printRequiredFieldErrorMessage(missingRequiredFields, projectRequiredFieldNotSet);
                    throw errorMessage;
                } else {
                    // try {
                    ...
                    paramObj = setProjectRelatedFields(project, entityObj);
        ...
        */
        function setProjectRelatedFields(project, entityObj) {
            log.debug('set project fields', project);
            var configOptions = ConfigOptions();
            // set primary fields
            project.setValue({ fieldId: 'custentity_bb_fin_prelim_purch_price_amt', value: entityObj.custentity_bb_fin_prelim_purch_price_amt });
            ...
            //project.setValue({ fieldId: 'custentity_bbss_enerflo_project_id', value: entityObj.custentity_bbss_enerflo_project_id });

            if (configOptions.usesubs) project.setValue({ fieldId: 'subsidiary', value: entityObj.subsidiary });
            ...
            var parent;
            if (!configOptions.useAddressRecord) {
                project.setValue({ fieldId: 'custentity_bb_homeowner_customer', value: entityObj.internalid });
                parent = addressRecord.createAddressCustomer(entityObj);
            } else {
            ...
            mapAdditionalCustomertoProjectFields(entityObj, project);

            return {
                templateId: entityObj.custentity_bb_started_from_proj_template,
                entityId: entityObj.internalid,
                proposalFileId: entityObj.custrecord_bb_file_system,
                utilityFileId: entityObj.custentity_bb_util_bill_doc_bb_file_sys
            };

        }


        // function mapAdditionalCustomertoProjectFields(entityObj, project) {
        //
        // 	var customrecord_bb_cust_to_proj_mappingSearchObj = search.create({
        // 		type: "customrecord_bb_cust_to_proj_mapping",
        // 		filters:
        // 			[
        // 				["isinactive","is","F"]
        // 			],
        // 		columns:
        // 			[
        // 				search.createColumn({name: "custrecord_bb_cust_to_proj_field_id", label: "Customer Field ID"}),
        // 				search.createColumn({name: "custrecord_bb_dest_cust_to_proj_field_id", label: "Project Field ID"})
        // 			]
        // 	});
        ...
        // 			id: entityObj.internalid,
        // 		})
        // 	}
        // 	customrecord_bb_cust_to_proj_mappingSearchObj.run().each(function(result){
        // 		var customerFieldId = result.getValue({name: 'custrecord_bb_cust_to_proj_field_id'});
        // 		var projectFieldId = result.getValue({name: 'custrecord_bb_dest_cust_to_proj_field_id'});
        // 		if (customerFieldId) {
        // 			var custValue = customer.getValue({fieldId: customerFieldId});
        // 			if (custValue && projectFieldId) {
        // 				project.setValue({fieldId: projectFieldId, value: custValue})
        // 			}
        // 		}
        // 		return true;
        // 	});
        ...
        */
        function getEntityRecordData(entityId, requiredFieldList) {
            var configOptions = ConfigOptions();
            var entityObj = {}
            var columnsForSearch = [
                ...
                //,search.createColumn({ name: "custentity_bbss_enerflo_project_id", label: "Enerflo Project Id" })
            ]
        ...
        */
        function createNewProjectActionsFromTemplate(templateProjectActionArr, toProjectId, utilityBillFileId, proposalFileId) {
            ...
            if (templateProjectActionArr.length > 0) {
                ...
                /*
                    A lot of fields on Project Action record are sourcing from other fields, and with the change to recmach approach, sourcing is not available
                    so we are going to create 1 Project Action instance in Dynamic mode that will never be saved to Netsuite, it will be just to set
                    field values and get the sourcing fields value, then set these fields
                    with recmach approach, reset the auxiliar Project Action instance and re use it. NOTE: This auxiliar record will never be saved.
                */
                var recProjectActionSourcingFields = record.create({
                    type: 'customrecord_bb_project_action',
                    isDynamic: true
                });
                 for (var i = 0; i < templateProjectActionArr.length; i++) {
                    ...
                    try{
                        //Setting field values on auxiliar Project Action record instance
                        recProjectActionSourcingFields = setSourcingFieldsOnAuxiliarProjectAction(recProjectActionSourcingFields, projActionObj);

                        //Setting fields on Project Action sublist on Project record
                        objProject.selectNewLine({
                            sublistId: 'recmachcustrecord_bb_project'
                        });
    
                        objProject.setCurrentSublistValue({
                            sublistId: 'recmachcustrecord_bb_project',
                        ...
                        objProject.setCurrentSublistValue({
                            ...
                            value: projActionObj.documentStatus
                        });
                        ...
                        log.debug('projActionObj.preceeding_package_action_id', projActionObj.preceeding_package_action_id);
                        log.debug('projActionObj.preceeding_package_action_id_from_package_action', projActionObj.preceeding_package_action_id_from_package_action);
                        
                        if (projActionObj.preceeding_package_action_id) {
                            objProject.setCurrentSublistValue({
                                sublistId: 'recmachcustrecord_bb_project',
                                fieldId: PROJECT_ACTION.PRECEEDING_PACKAGE_ACTION_ID,
                                value: projActionObj.preceeding_package_action_id.split(',')
                            });
                        } else if (projActionObj.preceeding_package_action_id_from_package_action) {
                            objProject.setCurrentSublistValue({
                                sublistId: 'recmachcustrecord_bb_project',
                                fieldId: PROJECT_ACTION.PRECEEDING_PACKAGE_ACTION_ID,
                                value: projActionObj.preceeding_package_action_id_from_package_action.split(',')
                            });
                        }

                        //Setting the sourcing fields on Project Action sublist on Project record, using the auxliar Project Action instance
                        objProject.setCurrentSublistValue({
                            sublistId: 'recmachcustrecord_bb_project',
                            fieldId: 'custrecord_bb_proj_act_related_adder_ite',
                            value: recProjectActionSourcingFields.getValue('custrecord_bb_proj_act_related_adder_ite')
                        });
                        objProject.setCurrentSublistValue({
                            sublistId: 'recmachcustrecord_bb_project',
                            fieldId: 'custrecord_bb_proj_act_docu_templ_id_txt',
                            value: recProjectActionSourcingFields.getValue('custrecord_bb_proj_act_docu_templ_id_txt')
                        });
                        objProject.setCurrentSublistValue({
                            sublistId: 'recmachcustrecord_bb_project',
                            fieldId: 'custrecord_bb_action_status_type',
                            value: recProjectActionSourcingFields.getValue('custrecord_bb_action_status_type')
                        });
                        objProject.setCurrentSublistValue({
                            sublistId: 'recmachcustrecord_bb_project',
                            fieldId: 'custrecord_bb_proj_act_sequence',
                            value: recProjectActionSourcingFields.getValue('custrecord_bb_proj_act_sequence')
                        });
                        objProject.setCurrentSublistValue({
                            sublistId: 'recmachcustrecord_bb_project',
                            fieldId: 'custrecord_bb_proj_act_show_rev_folders',
                            value: recProjectActionSourcingFields.getValue('custrecord_bb_proj_act_show_rev_folders')
                        });
                        objProject.setCurrentSublistValue({
                            sublistId: 'recmachcustrecord_bb_project',
                            fieldId: 'custrecord_bb_test_status',
                            value: recProjectActionSourcingFields.getValue('custrecord_bb_test_status')
                        });
                        objProject.setCurrentSublistValue({
                            sublistId: 'recmachcustrecord_bb_project',
                            fieldId: 'custrecord_bb_proj_act_overall_step_int',
                            value: recProjectActionSourcingFields.getValue('custrecord_bb_proj_act_overall_step_int')
                        });
                        objProject.setCurrentSublistValue({
                            sublistId: 'recmachcustrecord_bb_project',
                            fieldId: 'custrecord_bb_actgrp_sequence',
                            value: recProjectActionSourcingFields.getValue('custrecord_bb_actgrp_sequence')
                        });
                        objProject.setCurrentSublistValue({
                            sublistId: 'recmachcustrecord_bb_project',
                            fieldId: 'custrecord_bb_proj_action_item',
                            value: recProjectActionSourcingFields.getValue('custrecord_bb_proj_action_item')
                        });
                        objProject.setCurrentSublistValue({
                            sublistId: 'recmachcustrecord_bb_project',
                            fieldId: 'custrecord_bb_proj_action_msi_inspection',
                            value: recProjectActionSourcingFields.getValue('custrecord_bb_proj_action_msi_inspection')
                        });
                        objProject.setCurrentSublistValue({
                            sublistId: 'recmachcustrecord_bb_project',
                            fieldId: 'custrecord_bb_action_document_template',
                            value: recProjectActionSourcingFields.getValue('custrecord_bb_action_document_template')
                        });
                        objProject.setCurrentSublistValue({
                            sublistId: 'recmachcustrecord_bb_project',
                            fieldId: 'custrecord_bb_proj_act_counterparty_type',
                            value: recProjectActionSourcingFields.getValue('custrecord_bb_proj_act_counterparty_type')
                        });
                        //End setting sourcing fields

                        //Now commiting line to create Project Action record once Project is saved
                         objProject.commitLine({
                             sublistId: 'recmachcustrecord_bb_project',
                             ignoreRecalc: false
                         });

                         //Now reseting auxiliar Project Action instance so it can be used again on next loop. Reset is to make sure sourcing fields are empty at start.
                        recProjectActionSourcingFields = setSourcingFieldsOnAuxiliarProjectAction(recProjectActionSourcingFields);
                     }catch (e) {
                        log.error("ERROR leadToProject", "Failed to add project Action "+JSON.stringify(projActionObj));
                        log.error("ERROR", e);
                    }

                }
                objProject.save({ignoreMandatoryFields:true});
                /* Previous logic that does a record.create for every Project Action to be created
                for (var i = 0; i < templateProjectActionArr.length; i++) {
                    var projActionObj = templateProjectActionArr[i];
                    log.audit('project action object to be converted to new project', projActionObj);
                    try {
            ...
            newProjectActionArray = getTemplateRelatedProjectActions(toProjectId);
        ...
        function setSourcingFieldsOnAuxiliarProjectAction(recProjectActionSourcingFields, projActionObj){
            if(projActionObj){
                recProjectActionSourcingFields.setValue('custrecord_bb_package', projActionObj.package);
                recProjectActionSourcingFields.setValue('custrecord_bb_project_package_action', projActionObj.packageAction);
                recProjectActionSourcingFields.setValue('custrecord_bb_document_status', projActionObj.documentStatus);

                var actionItem = recProjectActionSourcingFields.getValue('custrecord_bb_proj_action_item');
                recProjectActionSourcingFields.setValue('custrecord_bb_proj_action_item', actionItem);
            }else{
                recProjectActionSourcingFields.setValue('custrecord_bb_proj_act_related_adder_ite', '');
                recProjectActionSourcingFields.setValue('custrecord_bb_proj_act_docu_templ_id_txt', '');
                recProjectActionSourcingFields.setValue('custrecord_bb_action_status_type', '');
                recProjectActionSourcingFields.setValue('custrecord_bb_proj_act_sequence', '');
                recProjectActionSourcingFields.setValue('custrecord_bb_proj_act_show_rev_folders', false);
                recProjectActionSourcingFields.setValue('custrecord_bb_test_status', '');
                recProjectActionSourcingFields.setValue('custrecord_bb_proj_act_overall_step_int', '');
                recProjectActionSourcingFields.setValue('custrecord_bb_actgrp_sequence', '');
                recProjectActionSourcingFields.setValue('custrecord_bb_proj_action_item', '');
                recProjectActionSourcingFields.setValue('custrecord_bb_proj_action_msi_inspection', '');
                recProjectActionSourcingFields.setValue('custrecord_bb_action_document_template', '');
                recProjectActionSourcingFields.setValue('custrecord_bb_proj_act_counterparty_type', '');
            }
            
            return recProjectActionSourcingFields;
        }


        function createMissingProjectActions(templateProjectActionArr, existingActions, toProjectId, utilityBillFileId, proposalFileId) {
            var newProjectActionArray = [];
            var packageActionObj = getConfigUtilityAndProposalPackageActionIds(null);
            if (templateProjectActionArr.length > 0) {
                var objProject = record.load({
                ...
                */
                ...
                for (var i = 0; i < templateProjectActionArr.length; i++) {
                    ...
                    try {
                        var actionIndex = existingActions.map(function (data) { return data.packageAction }).indexOf(projActionObj.packageAction);
                        log.debug('exiting project action index number', actionIndex);
                        if (actionIndex == -1) {
                            //Setting field values on auxiliar Project Action record instance
                            recProjectActionSourcingFields = setSourcingFieldsOnAuxiliarProjectAction(recProjectActionSourcingFields, projActionObj);
                ...
                for (var i = 0; i < templateProjectActionArr.length; i++) {
                    ...
                    try {
                        ...
                        log.error('error creating project action from template for project action object', projActionObj);
                    }
                }*/
            }
            newProjectActionArray = getTemplateRelatedProjectActions(toProjectId);
        ...
        */
        ...
        /*
         * function getProjectRequiredFieldRecords
         * @returns array - array or required field id values records
        */
        function getProjectRequiredFieldRecords() {
            var requiredFieldsArr = [];
            var customrecord_bb_req_fields_proj_creationSearchObj = search.create({
                type: "customrecord_bb_req_fields_proj_creation",
                filters: [
                    ['custrecord_bb_config_record', 'anyof', '1'],
                    'AND',
                    ['isinactive', 'is', 'F'],
                ],
                columns:
                    [
                        "custrecord_bb_required_field_txt",
                        "custrecord_bb_required_field_join_txt",
                        "custrecord_bb_reqrd_field_cust_err_txt"
                    ]
            });
            customrecord_bb_req_fields_proj_creationSearchObj.run().each(function (result) {
                requiredFieldsArr.push({
                    fieldId: result.getValue({ name: 'custrecord_bb_required_field_txt' }),
                    joinId: result.getValue({ name: 'custrecord_bb_required_field_join_txt' }),
                    errorMessage: result.getValue({ name: 'custrecord_bb_reqrd_field_cust_err_txt' })
                });
                return true;
            });
        ...
        */
        ...
        /*
         * function printRequiredFieldErrorMessage
         * @param {missingRequiredFieldArray} array - array of error messages to display.
         * @returns html - html string of a table containing error messages
        */
        function printRequiredFieldErrorMessage(missingRequiredFieldArray, isNoFieldSet) {
            ...
            if (isNoFieldSet) {
                responseLineHTML += '<body><table id= "items"><tr><th>The Project setup is not proper for REQUIRED FIELDS. Please contact you Administrator</th></tr>';

            } else {
                responseLineHTML += '<body><table id= "items"><tr><th>The following fields are required to create a project</th></tr>';

                for (var i = 0; i < missingRequiredFieldArray.length; i++) {
                    responseLineHTML += '<tr><td style= "white-space: nowrap";>' + missingRequiredFieldArray[i] + '</td></tr>';
                }
            }
        ...
        function addCustomerProjectAddersToProject(customerId, projectId){
            if(isEmpty(customerId) || isEmpty(projectId))
                return;
            ...
            var customrecord_bb_project_adderSearchObj = search.create({
                ...
                filters:
                    [
                        ["isinactive","is","F"],
                        "AND",
                        ["custrecord_bb_project_adder_project","anyof","@NONE@"],
                        "AND",
                        ["custrecord_bb_adder_customer","anyof",customerId]
                    ],
                columns:
                    [
                        search.createColumn({name: "internalid", label: "Internal ID"}),
            ...
            customrecord_bb_project_adderSearchObj.run().each(function (result) {
                ...
                record.submitFields({
                    ...
                    values: {
                        'custrecord_bb_project_adder_project': projectId
                    },
                    options: {
        ...
        return {
            ...
            createMissingProjectActions: createMissingProjectActions,
            createProjectByLotNumber: createProjectByLotNumber,
            addCustomerProjectAddersToProject: addCustomerProjectAddersToProject
        };
