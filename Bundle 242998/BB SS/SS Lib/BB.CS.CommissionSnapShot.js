...
define(['N/currentRecord', 'N/url', 'N/https', 'N/search'], function(currentRecord, url, https, search) {
    ...
    function pageInit(context) {
        ...
        var lineCount = currRecord.getLineCount({
            sublistId: 'custpage_snap_shot_list'
        });
        if (lineCount != -1) {
            for (var h = 0; h < lineCount; h++) {
                var jeRecord = currRecord.getSublistValue({
                    sublistId: 'custpage_snap_shot_list',
                    fieldId: 'custpage_journal_entry',
                    line: h
                });
                // highlight row if an addtion or clawback is found
                if (jeRecord) {
                    var trDom = document.getElementById('custpage_snap_shot_listrow'+h);
        ...
        var recordType = currRecord.getValue({fieldId: 'custpage_record_type'});
        var sublistId = '';
        if (recordType == 'CreateSnapShot' || !recordType) {
            sublistId = 'custpage_snap_shot_list'
        } else if (recordType == 'EditSnapShot') {
            sublistId = 'custpage_edit_snap_shot'
        } else if (recordType == 'DeleteSnapShot') {
            sublistId = 'custpage_delete_snap_shot_list'
        } else {
            sublistId = 'custpage_snap_shot_je_list'
        }
        console.log('record type', recordType);
        console.log('sublistid', sublistId);
        if (sublistId) {
            var lineCount = getLineCountFromSuitelet(sublistId);
            currRecord.setValue({
