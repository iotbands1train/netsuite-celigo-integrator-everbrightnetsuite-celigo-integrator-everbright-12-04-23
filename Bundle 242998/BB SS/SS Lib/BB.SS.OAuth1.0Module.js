            ...
            if (consumerKey) {
                ...
                if (method == 'POST') {
                    ...
                    if (postRsp.code == 200) {
                        ...
                        log.debug('post response body', postRsp.body);
                        log.debug('post response code', postRsp.code);
                    }
                }
            } else {
...
*/
    ...
    function requestDataObj(systemCreds) {
        ...
        log.debug('header obj', headerObj);
        return headerObj;
...
*/
    ...
    function signedHeaders(headerObj, path, method) {
        ...
        log.debug('param base string', paramsString);

        var urlDataEncoded = encodeURIComponent(paramsString);
        log.debug('url encoded param base string', urlDataEncoded);
        ...
        var test = 'test';
        var test2 = 'test2';
        var signedTest = CryptoJS.HmacSHA256(test, test2).toString(CryptoJS.enc.Hex);
        log.debug('hex value of test', signedTest);

        var signed = CryptoJS.HmacSHA256(signatureString, secret).toString(CryptoJS.enc.Hex); 
        log.debug('signed hex value', signed);
        var base64 = encode.convert({
            string: signed,
            inputEncoding: encode.Encoding.HEX,
            outputEncoding: encode.Encoding.BASE_64
        });
        log.debug('base64 signed string value', base64);
        var encodedSignature = encodeURIComponent(base64);
        log.debug('url encoded base64 signed string value', encodedSignature);
