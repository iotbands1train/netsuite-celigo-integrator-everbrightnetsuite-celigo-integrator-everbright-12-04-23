 ...
 * @fileOverview This UserEvent script is to trigger a Schedule Script for the Enerflo's integration
 *               main logic after a Project Interface record with an Enerflo ID is created.
 */

define(['N/runtime', 'N/task', 'N/search', 'N/query', 'N/record', 'SuiteBundles/Bundle 242998/BB SS/SS Lib/BB_SS_MD_SolarConfig', 'SuiteBundles/Bundle 242998/BB SS/SS Lib/BB.SS.ScheduledScript.BatchProcessing'],
(runtime, task, search, query, record, solarConfig, batchProcessing) => {
   ...
   const afterSubmit = (context) => {
      ...
      try{
         ...
         let enerfloId = objNewRecord.getValue('custrecord_bb_pi_enerflo_proj_id');
         if (!enerfloId) {return;}
         let enerfloPayload = JSON.parse(objNewRecord.getValue('custrecord_bb_pi_og_enerflo_payload'));
         let objData = getData(enerfloPayload, objNewRecord.id);
         if (!objData) {
            let errMsg = 'Error while generating data object from Enerflo payload.';
   ...
   const getData = (enerfloPayload, projectInterfaceId) => {
      let logTitle = 'getData';

      try{
         let contactData = enerfloPayload.contactData;
         let projectData = enerfloPayload.projectData;
         let v2DealData = enerfloPayload.v2DealData;
         ...
         let objData = {
            ...
            'customerData': {
               'customform': solarConfig.getConfiguration('custrecord_enerflo_cust_template').value,
               'category': solarConfig.getConfiguration('custrecord_enerflo_category').value,
               'entitystatus': solarConfig.getConfiguration('custrecord_enerflo_status_type').value,
               'subsidiary': 2, //TriSMART Solar
               'isperson': 'T',
               'custentity_bbss_enerflo_customer_id': contactData.enerfloCustomerId,
               'custentity_bbss_enerflo_project_id': projectData.enerfloProjectId,
               ...
               'custentity_bb_started_from_proj_template': solarConfig.getConfiguration('custrecord_enerflo_project_template').value,
               'custentity_bb_system_size_decimal': projectData.systemSize,
               'custentity_bb_roof_type': solarConfig.getConfiguration('custrecord_enerflo_roof_type').value,
   ...
   const getSalesRep = (objData) => {
      ...
      try{
         ...
         let EnerfloRepId = type == 'partner' ? objData.partner : objData.employee;

         log.debug(logTitle, EnerfloRepId + ' | ' + type);
         if (!EnerfloRepId || !type) {
            return 'Wrong Sales Rep Type or ID';
         }
         let entitySearch = search.create({
            type: type,
            filters: [['externalid', 'is', EnerfloRepId]],
