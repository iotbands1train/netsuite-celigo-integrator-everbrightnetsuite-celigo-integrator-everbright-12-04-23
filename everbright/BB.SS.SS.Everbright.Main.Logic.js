...
define(['N/runtime', 'N/record', 'N/query', 'N/search', 'N/error', 'SuiteBundles/Bundle 242998/BB SS/SS Lib/BB.SS.MD.LeadToProject.js'], (runtime, record, query, search, error, leadToProject) => {
    ...
    const execute = (context) => {
        ...
        let objData = JSON.parse(runtime.getCurrentScript().getParameter('custscript_obj_data'));
        let objErrors = [];
        try{
    ...
    const createCustomer = (customerData) => {
        ...
        if (customerExists != null) {
            ...
            objCustomerRecord = record.create({type: record.Type.LEAD, isDynamic: true});
        }
    ...
    const getLead = (everbrightCustId) => {
        ...
        try{
            ...
            if (projectInterfaceId) {
                ...
                let filesTable = search.lookupFields({
                    type: 'customrecord_bb_ss_project_interface',
                    id: projectInterfaceId,
                    columns: ['custrecord_bb_pi_everbright_proj_images']
                });

                if (filesTable && filesTable.custrecord_bb_pi_everbright_proj_images) {
                    fieldsToUpdate['custentity_everbright_proj_images'] = filesTable.custrecord_bb_pi_everbright_proj_images;
                }
            }
